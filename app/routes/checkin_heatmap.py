...
checkin_heatmap_bp = Blueprint('checkin_heatmap_api', __name__, url_prefix='/checkinHeatmap')
...
def genHeatMap(checkin_dates): 
   ...
   for date in checkin_dates.split(','):
        ...
        year_index = datetime_obj.year-2010            ##0 for 2010, 1 for 2011 and so on...
...
def testing():
    ...
    restaurant = Business().getBusiness(f={'name' : target_name,'address' : "3950 Las Vegas Blvd So"})
